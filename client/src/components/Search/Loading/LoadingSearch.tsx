...
const LoadingCards = ({
  ...
  const gridClassName =
    'grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 animate-pulse place-items-center';
...
export const LoadingCard = () => {
  const cardClassName = `relative h-47 w-77.5 ${styles.loadingMedia}`;
  return (
...
const LoadingCardContent = () => {
  return (
    <>
      <div className="absolute left-2 top-2 h-43 w-29.5 rounded-lg bg-gray-400/50" />
      ...
      <div className="absolute left-50.5 top-30 h-7 w-9 rounded-lg bg-gray-400/50" />
      <div className="absolute left-42 top-38 h-5 w-26 rounded-lg bg-gray-400/50" />
    </>
  );
};
